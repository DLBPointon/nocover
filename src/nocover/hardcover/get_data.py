...
class GetData:
    def __init__(self, query: str, api_path: str, config_path: Config, offline: bool):
        ...
        if api_path not in ["lists", "user_books", "profile", "prompts"]:
            ErrorModal(
                f"Dev made an error, they used {api_path} where only one the below is allowed:\n{allowed_api_paths}"
            )

        # Profile data is processed differently
        self.data = (
            get_data["me"][0] if api_path == "profile" else get_data["me"][0][api_path]
        )

    def format_tags(self, data: list[str]):
        """
        ...
        for list_slug in self.slugs:
            ...
            organised_dict[list_slug] = {
                "list_information": {
                    "slug": list_slug,
                    "name": list_data.get(
                        "name", list_data["question"]
                    ),  # Fall back is when prompts are being used.
                ...
                "book_list": self.corrected_book_list(
                    list_data.get("list_books", list_data["prompt_books"])
                ),
            }

        return organised_dict

    def corrected_book(self, book) -> dict[str, Any]:
        data = book["book"]
        return {
    ...
    def reorder_by_list_position(self, books):
        ...
        if position_data is False:
            ...
            return sorted(books, key=lambda x: x["list_position"], reverse=False)

    def corrected_book_list(self, book_list: list[dict[str, Any]]):
